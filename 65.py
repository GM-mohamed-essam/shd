...
TOKEN = "7445678934:AAHcK0tLMhwpB5cVbunaWGpmpoSaOEEj2lU"
bot = telebot.TeleBot(TOKEN)

def load_users():
    if os.path.exists('tsd.json'):
        with open('tsd.json', 'r') as f:
            return json.load(f)
    return []
...
def load_emails():
    if os.path.exists('i65.json'):
        with open('i65.json', 'r') as f:
            data = json.load(f)
            for user_id in data:
...
def save_emails(data):
    with open('i65.json', 'w') as f:
        json.dump(data, f, indent=4)

@bot.message_handler(commands=['start'])
def send_welcome(message):
    users = load_users()
    if str(message.from_user.id) in users or message.from_user.id == 7162066098: #ايديك
...
def handle_id(message):
    if message.from_user.id == 5973744090: #ايديك
